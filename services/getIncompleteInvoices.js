    ...
    invoiceData.then(async (res) => {
      ...
      doc.pipe(
        fs.createWriteStream(
          `factura-${nroFac}-${fr.nitCliente}-caja_${fr.puntoDeVenta + 1}.pdf`
        )
      );
      doc
        .font("Helvetica")
      ...
      doc
        .moveDown()
        .text(
          ` Son: ${convertido?.texto.toUpperCase()} CON ${convertido.resto}/100`
        )
        .text(" Bolivianos");
      ...
      invoiceUpdated
        .then((res) => {
          const emitida = updateIncompleteInvoices(fr.idFacturaIncompleta);
