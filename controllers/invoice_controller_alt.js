...
const getLeyendas = async (req) => {
  try {
    ...
    const random = Math.floor(
      Math.random() * (responseLeyenda.data.data.length - 1 + 1)
    );
...
const registerSalePos = async (data, idFactura, clientTemp) => {
  ...
  try {
    ...
    data.productos.map(async (producto) => {
      ...
      const totalProducto = total != undefined ? total : totalProd;
...
const createInvoicePostWithRetry = async (
  body,
  idsCreados,
  data,
  selectedLegend,
  ...
  while (retryCount < maxRetries) {
    try {
      const result = await createInvoicePost(
        body,
        idsCreados,
        data,
        selectedLegend,
...
const createInvoiceAltPlus = async (body, req) => {
  ...
  try {
    ...
    const typeStock = stockBody.idAlmacen.includes(
    ...
    for (const prod of stockBody.productos) {
      queries.push(
        updateStockQuery(typeStock, operator, prod, dateResult, stockBody)
      );
  ...
  try {
    if (Number(data.emission_type_code) === 1) {
      ...
      const responseFinal = await createInvoicePostWithRetry(
        body,
        idsCreados,
        data,
        selectedLegend
      ...
      const responseEstadoFactura = await getEstadoFactura(
        req,
        data.ack_ticket
      );
